...
SDK 0.2.0。用户取得诺诺确认：该账号使用 `sdk.nuonuo.com` 网关，但仅产生测试数据。
凭据由本地私密环境文件读取，未纳入仓库；请求号在提交前持久化。

| 检查 | 结果 |
| --- | --- |
| 诺税通列表查询 | HTTP 200，E0000 |
| 不存在的合成订单查询 | E9500，发票不存在 |
| 单张数电普通蓝票提交 | E0000，返回开票流水号 |
| 按原订单号查询最终状态 | E0000，status=2，订单和销方税号匹配 |
| 金额 | 不含税 1.00，税额 0.06，价税合计 1.06 |
| PDF / OFD 下载 | 分别 57183 / 23482 字节，文件头检查通过 |
| Odoo 协议映射 | 真实返回通过 protocol v1 身份及金额校验 |

本次仅提交一张测试票，未压测、未冲红或作废、未请求短信或邮件交付。
完整回执留在私密本地记录中，本仓库不保存账号、税号、票号、令牌或下载链接。
该结果证明上述普通蓝票链路可用；专票、红字及生产企业授权仍需各自验收。
